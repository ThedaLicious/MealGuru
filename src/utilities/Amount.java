...
public class Amount {
	 ...
	 *
	 * @param measure the parameter measure
	 * @param unitMeasurement the parameter for unit clarification
	 */
	...
	/**
	 * Constructor used for database and also simple creation of an Amount.
	 * This constructor is useful because all you need to enter into it is
	 * a {@link String} (e.g. "33 fl.oz", "44 g", "4 unit").
	 *
	 * It handles separating the measurement and the {@link Units} by
	 * splitting the {@link String} then setting the class members: double measure,
	 * and {@link Units} unitMeasurement.
	 *
	 * @param amount this is a {@link String} argument of what you want to convert to an amount
	 */
	public Amount(String amount) {

		String [] broken = amount.split(" ");

		this.setMeasure(Double.parseDouble(broken[0]));

		if(broken[1].equalsIgnoreCase("unit")) {
	 ...
	 *
	 * @param toSet the double amount to set this {@link Object} to.
	 */
	...
	/**
	 * This method's purpose is to add an {@link Amount} to
	 * this Amount. It takes into consideration the {@link Units}.
	 *
	 * This method cannot convert between liquid {@link UnitClassification}'s,
	 * solids, and utilities.
	 *
	 * @param amountToAdd This method takes an {@link Amount} to add. The result
	 *                    is kept in the {@link Units} that is being added to.
	 */
	public void add(Amount amountToAdd) {
		...
		if(amountToAdd.getUnits() != this.getUnits()) {
			if(this.getUnits().getUnitClassification() == amountToAdd.getUnits().getUnitClassification()) {
				...
				System.out.println("Cannot add Amounts: " + this.getUnits().getUnitClassification() + " -!-> " + amountToAdd.getUnits().getUnitClassification());
			}
		} else {
			this.measure += amountToAdd.measure;
		}
	 ...
	 *
	 * Conversions are IMPOSSIBLE between Amounts with different {@link UnitClassification}'s.
	 * <i>Units.GRAM -!- Units.FLUID_OUNCE</i>
	 * <i>Units.UNIT -!- Units.CUP</i>
	 *
	 * @param toThisUnit this is the {@link Units} to convert to.
	 */
	...
	public String toString() {
		...
		return measure + " " + getUnits().getAbbreviation();
